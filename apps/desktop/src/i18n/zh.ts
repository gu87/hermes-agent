...
export const zh: Translations = {
  ...
  boot: {
    ready: 'Seven Desktop 已就绪',
    desktopBootFailedWithMessage: message => `桌面启动失败:${message}`,
    steps: {
      connectingGateway: '正在连接实时桌面网关',
      loadingSettings: '正在加载 Seven 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 Seven Desktop…'
    },
    errors: {
      backgroundExited: 'Seven 后台进程已退出。',
      backgroundExitedDuringStartup: 'Seven 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Seven 后端早于当前桌面构建,可能无法正常工作。请更新以保持一致。',
    updateHermes: '更新 Seven',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求(405 Method Not Allowed)。请尝试重启 Seven Desktop。',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Seven 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      gateway: '网关',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Seven 跟随系统设置。',
    ...
    fieldDescriptions: {
      ...
      timezone: '当 Seven 需要本地时间上下文时使用。留空则使用系统时区。',
      ...
      file_read_max_chars: 'Seven 单次文件读取可读取的最大字符数。',
      'approvals.mode': 'Seven 如何处理需要显式审批的命令。',
      ...
      'agent.max_turns': 'Seven 停止一次运行前工具调用轮次的上限。'
    },
    about: {
      heading: 'Seven Desktop',
      ...
      automaticUpdatesDesc: 'Seven 会在后台自动检查更新,并在有可用更新时通知你。',
  ...
  agents: {
    ...
    gatewayInfo: (version, sessions) => `v${version} · ${sessions} 个活跃会话`,
    recentLogs: '最近日志',
    noLogs: '暂无日志。',
    systemNoData: '系统状态暂不可用。',
    refresh: '刷新',
    refreshing: '刷新中…',
    emptyTitle: '暂无活跃子代理',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Seven 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `Seven ${version} · 活跃会话 ${count}`,
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接,然后把 Seven 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Seven 自带的 WhatsApp 桥接,首次运行时扫描二维码,然后启用该平台。',
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server,暴露其 API,然后用服务器密码把 Seven 指向该 URL。',
      ...
      weixin: '登录微信公众平台,复制 AppID 和 Token,并把消息回调 URL 指向 Seven。',
      qqbot: '在 QQ 开放平台(q.qq.com)注册一个应用,复制 App ID 和 Client Secret。',
      api_server: '把 Seven 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥,然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Seven 环境:各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
  ...
  cron: {
    ...
    emptyDescNew: '按 cron 表达式排程一个提示词。Seven 会运行它,并把结果发送到你选择的目的地。',
  ...
  composer: {
    message: '消息',
    placeholderStarting: '正在启动 Seven…',
    placeholderReconnecting: '正在重新连接 Seven…',
    ...
    attachUrlDesc: 'Seven 将抓取该页面并作为本回合的上下文。',
