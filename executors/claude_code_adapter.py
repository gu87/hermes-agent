#!/usr/bin/env python3
"""
Claude Code adapter — runs ``claude-code`` as a subprocess.

Minimal v0.3 implementation:
  - Command launch via asyncio subprocess
  - stdout/stderr log collection
  - Exit status reporting
  - Health check via ``which claude-code && claude-code --version``

Not implemented (v0.4+):
  - Structured JSON line streaming (requires ``--output-format stream-json``)
  - Diff generation (requires git snapshot tracking)
  - Review gate
  - Worktree support
...
# Default command name — can be overridden via ExecutorConfig.path
...
class ClaudeCodeAdapter:
    ...
    async def check_health(self) -> ExecutorHealthResult:
        """Check that ``claude-code`` exists and is runnable."""
        command = _resolve_command(None)
        found, path = await check_command_exists(command)
        if not found:
            return make_unavailable_health(
                "claude-code",
                f"Command not found: '{command}' — install via: npm install -g @anthropic-ai/claude-code",
            )

        version = await check_version(command, DEFAULT_VERSION_ARGS)
        return make_available_health("claude-code", version=version)
    ...
    async def start(
        ...
        The process receives the prompt via stdin and runs to completion.
        stdout and stderr are collected as log events.
        """
        ...
        # Build command line
        cmd = [command, "--print", "--dangerously-skip-permissions"]
        if run.model_ref:
            cmd.extend(["--model", run.model_ref])
        ...
        try:
            ...
            if state.process.stdin:
                state.process.stdin.write(run.prompt.encode("utf-8"))
            ...
            asyncio.create_task(
                ...
                name=f"claude-code-collect-{run_id[:8]}",
            )
    ...
    async def _collect_output(self, state: _RunState) -> None:
        """Read stdout/stderr until process exits, normalize to events."""
    ...
    async def stop(self, run_id: str) -> None:
        """Send SIGTERM to the subprocess. Idempotent."""
    ...
    async def stream_events(self, run_id: str) -> AsyncIterable[RunEvent]:
        """Yield buffered events from the subprocess run."""
        state = self._runs.get(run_id)
        if state is None:
            yield RunEvent(
                type=RunEventType.FAILED,
...
def _resolve_command(override: Optional[str]) -> str:
    """Resolve the command path. If override is given, use it; else default."""
    if override:
        return override
    # Check for env override
    env_cmd = os.environ.get("HERMES_CLAUDE_CODE_PATH")
    if env_cmd:
        return env_cmd
